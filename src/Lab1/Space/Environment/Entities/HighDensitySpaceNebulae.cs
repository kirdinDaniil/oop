...
{
    public HighDensitySpaceNebulae(EnvironmentLenght lenght)
    {
        PathLenght = lenght;
    ...
    public ImpactResult? ImpactOnSpaceship(ISpaceship spaceship)
    {
        ...
        if (spaceship.JumpEngine is null || spaceship.JumpEngine.MaxJumpLenght < (double)PathLenght)
            return new ImpactResult() with { IsSpaceShipLost = true };
